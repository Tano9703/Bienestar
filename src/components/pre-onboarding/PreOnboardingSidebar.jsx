    ...
    <div className="flex flex-col h-full bg-white p-4">
        <div className="flex justify-between items-center mb-6 p-2">
            <Link to="/" className="flex-shrink-0">
                <img alt="CrossLearning Logo" className="h-10 w-auto" src="https://horizons-cdn.hostinger.com/0e1f7dd3-06a3-49ec-a3f5-eb9a3a70d9a6/f37727e20374c5671d0f7193f8b3d12a.png" />
            </Link>
            {onClose && (
        ...
        <nav className="flex-1 space-y-2">
            {preOnboardingSections.map((section) => (
                <NavItem
                    key={section.id}
                    section={section}
...
const PreOnboardingSidebar = ({ isOpen, setIsOpen, currentSectionId }) => {
    const navigate = useNavigate();

    const handleSelectSection = (sectionId) => {
        navigate(`/pre-onboarding/${sectionId}`);
        setIsOpen(false);
    };
...
export default PreOnboardingSidebar;